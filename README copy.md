...
## 2. 启动项目开发
    使用reactcreate-react-app(脚手架)搭建项目 
    1). create-react-app reacT-admin :下载模板项目
    2)开发环境运行: 
       npm start
    2)生产环境打包运行: 
       npm run build   默认只生成本地的打包文件
       本地测试运行需要用到静态服务器的包 npm install -g serve
       serve build
...
## 3. git管理项目
    ...
    6). 新的同事: 克隆仓库
        git clone url
        git checkout -b dev origin/dev 从远程的dev分支产生本地的dev分支,同上
        git pull origin dev
    7). 如果远程修改
        git pull origin dev
...
## 5 引入antd
    ...
    npm i react-app-rewired customize-cra -S  并修改 package.json 里的启动配置
        package.json 内容修改如下:
        /* package.json */
        "scripts": {
    ...
    npm i babel-plugin-import -S 用于按需加载组件代码和样式的 babel 插件

      在项目根目录创建一个 config-overrides.js 用于修改默认配置
        config-overrides.js 内容如下 : 最终的配置

        const { override, fixBabelImports,addLessLoader } = require('customize-cra');
        module.exports = override(
            // 使用 针对 antd 实现按需打包, 根据 import 来打包 (使用 babel-plugin-import)
            ...
            // 使用Less-Loader对源码中的 Less 的变量进行重新指定
            addLessLoader({
    ...
    自定义主题
       npm i less less-loader -S 同时修改 config-overrides.js 文件如上:
    使用antd的组件
        根据antd的文档编写
...
## 6. 引入路由
    ...
    注册路由:
      ...
      注意: 哪怕为 路由启用了exact 精确匹配模式, 也会从上到下, 把所有的路由规则匹配一遍
      使用: 路由中的 Switch 组件, 能够指定, 如果前面的路由规则优先匹配到了, 则放弃匹配后续的路由
      <Switch> 
             HashRouter 表示一个路由的 根容器, 将来, 所有的路由相关的东西, 都要包裹在 HashRouter 里面, 而且, 一个网站中, 只需要使用一次 HashRouter 就好了
             ...
             如果要匹配参数, 可以在匹配规则中, 使用 : 修饰符, 表示这个位置匹配到的是 参数           
             注意: 如果想要从路由规则中, 提取参数, 需要使用 this.props.match.params
      <Route path='' component={}/>
      HashRouter: 不支持携带对象的 方式, 只能通过参数的形式携带字符串, 只能携带 商品id,  那还要
      在那边请求后台获取对应的产品, 是比较浪费掉的
## 7. Login的静态组件
    1). 自定义了一部分样式布局
    2). 使用antd的组件实现登陆表单界面
      Form  
...
## 8. 收集表单数据和表单的前台验证
    ...
    2). 操作表单数据
        ...
        onFinish={this.onFinish} 得到包含所有输入数据的对象
        ...
        a. 声明式实时表单验证:
          ...
          //声明式验证：直接使用别人定义好的验证规则进行验证
           rules={[ // 配置对象:属性名是特定的一些名称
               { required: true,whitespace:tr   message: '用户名必须输入' },
               { min: 4, message: '用户名至少4位' },
               { max: 12, message: '用户名最多12位' },
               { pattern: /^[a-zA-Z0-9_]+$/, message   户名必须是英文,数字或下划线组成'},
               ]} >
        b. 自定义表单验证
            3.xxx版本
...
## 2. 编写ajax代码
    ...
    5). async 异步 和 await 等待
        ...
        b. 哪里写await?
           不想要promise, 想要promise异步执行的成功的response数据
            在返回promise的表达式左侧写await
            const response =await reqLogin(username, password)
        c. 哪里写async?
            await所在函数(最近的)定义的左侧写async
            async 后面箭头函数前面的参数即使为一个, 也要加小括号,不然得到 的 数据为undefined
...
## 5. LeftNav组件
    ...
    4). 根据动态生成Item和SubMenu的数组
        map() + 递归: 多级菜单列表
        reduce((pre,item)=>{ ... return pre},[]) + 递归: 多级菜单列表
...
## 1. Header组件
    ...
    6). 退出登陆
        ...
        跳转到login
    7). 抽取通用的类链接按钮组件
        通过...透传所有接收的属性: <Button {...props} />    <LinButton>xxxx</LinkButton>
...
## 5. 更新分类
    ...
    2). 功能
        父组(Category)件得到子组件(AddForm)的数据(通过在父组件中创建回调函数)
        通过 ref 获取数据域 formRef = React.createRef();
        通过 this.formRef.current.func 调用相关事件
       ...
       <Form ref={this.formRef} onValuesChange={this.onValuesChange}> onValuesChange 属性 用于 字段值更新时触发回调事件
        <Select onChange={this.onCurrencyChange}  > onChange 为监听事件, 被调用函数监听到数据变化后, 会动态更改 name 的属性值
...
## 1. 添加分类
    ...
    2). 功能
        父组(Category)件得到子组件(AddForm)的数据(回调函数)
        调用添加分类的接口
        重新获取分类列表
...
## 4. ProductHome组件
    ...
    4). 进入详情界面
       ...
       注意:  <BrowserRouter> 不带井号  支持以上语法
              <HashRouter> 带井号 不支持以上语法  需要以/id的方式去后台查询
...
## 5. ProductDetail组件
    1). 读取商品数据: this.props.location.state.product  (注意: 如果传过来的product没有加大括号,name这里就不需要.product了)
    2). 显示商品信息: <Card> / List 
    3). 异步显示商品所属分类的名称
